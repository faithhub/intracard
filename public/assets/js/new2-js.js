...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function validateCurrentStep(type, divId = null) {
        // console.log(type);

        // Declare currentContent outside the conditional block
        let currentContent;

        if (type === "main") {
            currentContent = steps[currentStep];
            // console.log(currentContent, "currentContent (main)");
        } else if (type === "subStep") {
            // Add logic for subStep if needed
            currentContent = divId;
            // console.log(currentContent, "currentContent (subStep)");
        }
        else {
            console.log(currentContent, "currentContent (none)");
            return false;
        }
        ...
        console.log(currentContent, "currentContent in validation");
    ...
    function proceedToNextDiv(currentDiv) {
        // Get the div ID from currentDiv
        const currentDivId = currentDiv.id;

        // Check if the currentDiv is defined in stepFlow
        const currentStepConfig = Object.values(stepFlow).find(step => step.divId === currentDivId);

        if (currentStepConfig) {
            // Get the input name and find the selected option's value
            const inputName = currentStepConfig.inputName;
            ...
            if (selectedOption) {
                ...
                // Determine the next step based on the selected value
                const nextStepId = currentStepConfig.next(selectedValue);
                console.log(`Determined nextStepId: ${nextStepId}`);

                // Hide all steps in this group before showing the next one
                hideAllSubSteps();

                // Display the next step div if it exists
                const nextStepDiv = document.getElementById(nextStepId);
                if (nextStepDiv) {
                    nextStepDiv.style.display = "block";
                    console.log(`Proceeding to next div: ${nextStepId}`);
                    return nextStepDiv; // Return the next step div
                } else {
                    console.error(`Next div with ID '${nextStepId}' not found.`);
                }
            } else {
                console.warn(`No option selected for ${inputName}. Please make a selection.`);
            }
        } else {
            console.warn(`Current div '${currentDivId}' is not in the stepFlow configuration.`);
        }

        return null; // Return null if no next step was displayed
    }


    // Helper function to hide all steps in the current flow
    ...
    function moveNextStep() {
        // Function to get the currently displayed div with `display: block`
        ...
        function getCurrentDisplayedDiv() {
            const contentDivs = document.querySelectorAll('[data-kt-stepper-element="content"]');
            const displayedDiv = Array.from(contentDivs).find(div => div.style.display === "block");

            if (displayedDiv) {
                // Check if the displayed div has data-step-form="true"
                const hasStepForm = displayedDiv.getAttribute('data-step-form') === "true";
                console.log(`Displayed div ID: ${displayedDiv.id}, data-step-form: ${hasStepForm}`);

                return {
                    div: displayedDiv,
                    hasStepForm: hasStepForm // true or false based on the attribute presence
                };
            }

            return null; // Return null if no displayed div is found
        }


        // Get the currently displayed div
        // let currentDiv = getCurrentDisplayedDiv();
        const currentDisplayedInfo = getCurrentDisplayedDiv();
        if (currentDisplayedInfo) {
            const { div, hasStepForm } = currentDisplayedInfo;
            console.log(`Current div ID: ${div.id}, Has data-step-form="true": ${hasStepForm}`);
    
            let currentDiv = div;
        if (!hasStepForm) {
            console.log(currentDiv);
            
            // Check if the current div is part of a step flow by `data-step-form`
            if (currentDiv.getAttribute('data-step-form') === "true") {
                // Find the inner div that matches any stepFlow entry
                console.log(currentDiv, "currentDiv as div");

                const innerDiv = Array.from(currentDiv.querySelectorAll("div[id]"))
                    .find(div => Object.values(stepFlow).some(step => step.divId === div.id));

                if (innerDiv) {
                    // Get the ID of the matching inner div
                    const innerDivId = innerDiv.id;
                    console.log(`Processing inner div with ID: ${innerDivId}`);

                    // First, determine the next div and update currentDiv
                    const nextDiv = proceedToNextDiv(innerDiv);
                    if (nextDiv) {
                        currentDiv = nextDiv; // Update currentDiv to the newly displayed div
                        console.log(`Updated to next div: ${currentDiv.id}`);
                    } else {
                        console.warn("No next div found after determining the next step. Staying on current div.");
                        return; // Halt if there is no next div to display
                    }

                    // Now validate the updated currentDiv
                    if (!validateCurrentStep("subStep", currentDiv)) {
                        console.log("Validation failed for the updated currentDiv.");
                        return; // Halt if validation fails
                    }

                    console.log(`Successfully validated and moved to div: ${currentDiv.id}`);
                    return; // Halt further action once the next div is displayed and validated
                }

                else {
                    console.log("No matching step flow ID found within the current div.");
                }
            } else {
                // Validate the main step if not `data-step-form="true"`
                if (!validateCurrentStep("main")) {
                    console.log("Validation failed for main step.");
                ...
                console.log("Moved to the next main step.");
            }
        }
    }
    }
